...
class ThermalWrapper : public rclcpp::Node {
  ...
  private:
    ...
    bool
    transformContours(const image_geometry::PinholeCameraModel model,
                      const std_msgs::msg::Header header,
                      const std::vector<std::vector<cv::Point>> &contours,
                      std::vector<std::vector<geometry_msgs::msg::PointStamped>> &map_contours);
    bool getImagePointsInGPS(const std::vector<cv::Point3d> &centers,
                             const std_msgs::msg::Header &header,
                             std::vector<geometry_msgs::msg::Point> &gps_centers);
    bool transformCVPoint(const cv::Point3d point,
                          const geometry_msgs::msg::TransformStamped transform_image2map,
