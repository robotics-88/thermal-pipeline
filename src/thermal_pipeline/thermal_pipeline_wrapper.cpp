...
void ThermalWrapper::initialize() {
    ...
    sync_.reset(new message_filters::Synchronizer<approximate_policy>(
        approximate_policy(30), thermal_cam_subscriber_, thermal_info_subscriber_,
        secondary_cam_subscriber_, secondary_info_subscriber_));
    sync_->registerCallback(std::bind(&ThermalWrapper::thermalImgCallback, this,
                                      std::placeholders::_1, std::placeholders::_2,
                                      std::placeholders::_3, std::placeholders::_4));

    thermal_contour_pub_ = this->create_publisher<sensor_msgs::msg::Image>("/thermal_contours", 10);
    second_contour_pub_ =
        this->create_publisher<sensor_msgs::msg::Image>("/secondary_contours", 10);
...
void ThermalWrapper::thermalImgCallback(
    ...
    bool got_tf =
        transformContours(thermal_model_, img->header, thermal_contours, thermal_contours_map);
    if (!got_tf) {
    ...
    if (use_rviz_) {
        ...
        thermal_contour_msg.header =
            second_img->header; // Same timestamp and tf frame as input image
...
bool ThermalWrapper::transformContours(
    const image_geometry::PinholeCameraModel model, const std_msgs::msg::Header header,
    const std::vector<std::vector<cv::Point>> &contours,
    std::vector<std::vector<geometry_msgs::msg::PointStamped>> &map_contours) {
    geometry_msgs::msg::TransformStamped transform_thermal2map;
    std::string transform_error;
    try {
        transform_thermal2map =
            tf_buffer_->lookupTransform(map_frame_, header.frame_id, header.stamp);
...
bool ThermalWrapper::transformCVPoint(
    const cv::Point3d point, const geometry_msgs::msg::TransformStamped transform_image2map,
    const std_msgs::msg::Header header, geometry_msgs::msg::PointStamped &map_point) {
...
bool ThermalWrapper::getImagePointsInGPS(const std::vector<cv::Point3d> &centers,
    ...
    for (int i = 0; i < centers.size(); i++) {
        ...
        const std::shared_ptr<rclcpp::Node> geo_srv_node =
            rclcpp::Node::make_shared("geo_srv_client");
        if (rclcpp::spin_until_future_complete(geo_srv_node, geo_resp) ==
